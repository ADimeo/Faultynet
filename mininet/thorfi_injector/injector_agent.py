...
class Injector:
    ...
    def __init__(self,
                 target_nics=None,  # user-provided. Interfaces, like eth0
                 target_node=None,
                 # user-provided, generated from name, mininet node class, where to run injection code
                 fault_target_traffic=None,
                 # user-provided, if fault target is not ANY_TRAFFIC generate cmds for injecting according to protocol and port number
                 ...
                 interface_owner=None): # use-provided, generated by faultcontroller. Knows how I need to inject into my interface
    ...
    async def _inject_burst(self):
        ...
        for i in range(burst_num):
            ...
            for interface in self.getTargetInterfaces():
                ...
                self.inject_nics(interface, self.target_node,self.getFaultType(), 'persistent', [''], self.getFaultArgs(),
    ...
    async def _inject_degradation(self):
        ...
        for i in range(int(self.getInjectionTime())):
            ...
            for interface in self.getTargetInterfaces():
                ...
                self.inject_nics(interface,self.target_node ,self.getFaultType(), 'random', [degradation_value],
    ...
    async def _inject_default(self):
        if 'delete' in self.getFaultType():
            ...
            for interface in self.getTargetInterfaces():
                ...
                self.inject_nics(interface, self.target_node, self.getFaultType(), self.getFaultPattern(),
    ...
    def make_nics_injection_command(self, device, fault_type, fault_pattern, fault_pattern_args, fault_args, tc_cmd):
        log.debug(
        ...
        if 'random' in fault_pattern:
            ...
            if 'delay' in fault_type:
                ...
                command = tc_path + '/tc qdisc ' + tc_cmd + ' dev ' + device + ' root netem ' + fault_type + ' ' + fault_args + ' reorder ' + str(
                    random_perc) + '%'

            else:
                # in that case for corruption and loss we can use the 'fault_args' that already include random probability
                command = tc_path + '/tc qdisc ' + tc_cmd + ' dev ' + device + ' root netem ' + fault_type + ' ' + fault_pattern_args + '%'
            ...
            if 'delay' in fault_type:
                ...
                command = tc_path + '/tc qdisc ' + tc_cmd + ' dev ' + device + ' root netem ' + fault_type + ' ' + fault_args
                ...
                command = tc_path + '/tc qdisc ' + tc_cmd + ' dev ' + device + ' root tbf rate ' + fault_args + 'kbit burst ' + default_bottleneck_burst + ' limit ' + default_limit_burst
                ...
                command = tc_path + '/tc qdisc ' + tc_cmd + ' dev ' + device + ' root netem ' + fault_type + ' 100%'

        return command

    def make_filter_cmds(self, fault_pattern, fault_pattern_args, fault_type, fault_args, device, target_protocol,
        ...
        if enable:
            ...
            if target_dst_ports:
                for target_port in target_dst_ports:
                    ...
                    cmd_list.append(
                        tc_path + '/tc filter add dev ' + device + ' parent 1:0 protocol ip prio 1 u32 ' + target_protocol_cmd + ' ' + target_port_cmd + ' flowid 1:1')
            ...
            if target_src_ports:
                ...
                cmd_list.append(
                    tc_path + '/tc filter add dev ' + device + ' parent 1:0 protocol ip prio 1 u32 ' + target_protocol_cmd + ' flowid 1:1')
            ...
            if 'random' in fault_pattern:
                ...
                if 'delay' in fault_type:
                    ...
                    random_perc = 100 - int(fault_pattern_args)
                    cmd_list.append(
                        tc_path + '/tc qdisc add dev ' + device + ' parent 1:1 handle 2: netem ' + fault_type + ' ' + fault_args + ' reorder ' + str(
                            random_perc) + '%')
                else:
                    cmd_list.append(
                        tc_path + '/tc qdisc add dev ' + device + ' parent 1:1 handle 2: netem ' + fault_type + ' ' + fault_pattern_args + '%')
                ...
                if 'bottleneck' in fault_type:
                    ...
                    default_limit_burst = '3000'
                    cmd_list.append(
                        tc_path + '/tc qdisc add dev ' + device + ' parent 1:1 handle 2: tbf rate ' + fault_args + 'kbit burst ' + default_bottleneck_burst + ' limit ' + default_limit_burst)
                    ...
                    cmd_list.append(
                        tc_path + '/tc qdisc add dev ' + device + ' parent 1:1 handle 2: netem ' + fault_type + ' ' + tc_arg)

        else:
            cmd_list = []
            cmd_list.append(tc_path + '/tc qdisc del dev ' + device + ' root handle 1: prio')

        log.debug("cmd_list generated => %s\n" % cmd_list)

        return cmd_list

    def inject_nics(self, device, target_node, fault_type, fault_pattern, fault_pattern_args, fault_args, target_traffic,
        ...
        if 'any' in target_traffic:
            ...
            if enable:
                # enable fault injection
                command = self.make_nics_injection_command(device, fault_type, fault_pattern, fault_pattern_args[0],
                                                           fault_args, 'add')
            else:
                command = self.make_nics_injection_command(device, fault_type, fault_pattern, fault_pattern_args[0],
                                                           fault_args, 'del')

            log.debug("Execute command on host %s: '%s'\n" % (target_node.name,command))

            # retcode = call(command, shell=True)  # TODO call the relevant hosts command instead
            # command_output = target_node.cmdPrint(command)
            command_output = target_node.popen(command)
            log.debug("Command '%s' led to  (retcode %s)\n" % (command, command_output))
            #
            #
            # if retcode < 0:
        ...
        # if fault target is not ANY_TRAFFIC generate cmds for injecting according to protocol and port number
            ...
            if enable:
                cmd_list = self.make_filter_cmds(fault_pattern, fault_pattern_args[0], fault_type, fault_args, device,
